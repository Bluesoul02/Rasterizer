    ...
    class Scene {
        ...
        public:
                ...
                while( this->running )
                {
                    ...
                    for (Shape s : shapes) {
                        for (Face f : s.get_faces()) {
                            ...
                            Vertex vert0 = s.get_vertices()[f.get_v0()];
                            Vertex vert1 = s.get_vertices()[f.get_v1()];
                            Vertex vert2 = s.get_vertices()[f.get_v2()];
                            Vec2r v0 = vert0.get_vector();
                            Vec2r v1 = vert1.get_vector();
                            Vec2r v2 = vert2.get_vector();
                            if (display == 0) {
                                ...
                                // draw_shaded_triangle(v0, vert0.get_H(), v1,vert1.get_H(), v2, vert2.get_H());
                            }
                        }
                    }
            ...
            void draw_shaded_triangle( const Vec2r & v0, real h0, const Vec2r & v1, real h1, const Vec2r & v2, real h2 ) const {
                ...
                std::vector<real> x12 = interpolate(y1, x1, y2, x1);
                std::vector<real> h12 = interpolate(y1, h1, y2, h1);
                ...
                h01. pop_back();
                std::vector<real> h012(h01);
                h012.insert(h012.end(), h12.begin(), h12.end());

                int m = floor(x012.size() / 2);
                std::vector<real> x_left;
                std::vector<real> x_right;
                std::vector<real> h_left;
                std::vector<real> h_right;
                if(x02[m] < x012[m]) {
                    x_left = x02;
                    x_right = x012;
                ...
                minwin::Color real_color; 
                Vector<real, 3> color = { minwin::RED.r, minwin::RED.g, minwin::RED.b};
                Vector<real, 3> shaded_color; 
                for(int y = y0; y <= y2; ++y)
                {
                    x_l = x_left[y - y0];
                    // x_r = x_right[y - y0];
                    ...
                    for(int x = x_left [y - y0]; x <= x_right [y - y0]; ++x)
                    {
                        ...
                        real_color = {shaded_color[0], shaded_color[1], shaded_color[2], 0};
                        window.put_pixel (x, y, real_color);
                    }
                }
            }
                ...
                public:
                    ...
                    void on_press() const {
                        if (this->owner.display == 0) {
                            ...
                            this->owner.display = 2;
                        }
                        else if (this->owner.display == 2) {
